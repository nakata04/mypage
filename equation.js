...
function graphSheet(Xmin, Xmax, Ymin, Ymax) {
  ...
  for (i = offsetX; i < maxWidth; i += 40) {
    if (!(i == offsetX)) {
      ...
      ctx.fillText(Math.round(numX * 100) / 100, i, offsetY + 5);
    }
  }
  numX = 0;
  ...
  for (i = offsetY; i < maxHeight; i += 40) {
    if (!(i == offsetY)) {
      ...
      ctx.textAlign = "right";
      ctx.fillText(Math.round(numY * 100) / 100, offsetX - 5, i + 5);
    }
  }
  numY = 0;
...
function reset(){
    answerValue11.innerHTML="";　//リセット
    answerValue21.innerHTML ="";
    interception.innerHTML="";
}

let slv = document.getElementById("solve");
slv.addEventListener('click',function(){

    reset();

    var a = parseFloat(document.myFORM.myC1.value);
    var b = parseFloat(document.myFORM.myC2.value);
    var c = parseFloat(document.myFORM.myC3.value);
    ...
    if (d > 0){　//異なる実数解を二つ持つ
        ...
        answerValue11.innerHTML = root1;
        answerValue21.innerHTML = root2;
        interception.innerHTML  = root3;
        ...
        var realPart = -b / (2*a); //実部
        var iPart = Math.sqrt(-d) / (2 * a);　//虚部
        root3 = c;

        answerValue11.innerHTML = realPart + "+" + iPart + " i";
        answerValue21.innerHTML = realPart + "-" + iPart + " i";
        interception.innerHTML  = root3;
    }
    ...
    document.getElementById('answerMessage1').style.display = 'block';　//解を表示
})
